...
import services.{AccountingService, MetricsService, SessionCacheService, TimeService}
...
class AccountingDatesController @Inject() (val authConnector: PlayAuthConnector,
                                           ...
                                           view: AccountingDatesView)(implicit val appConfig: AppConfig, implicit val ec: ExecutionContext)
  ...
  val show: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      checkStatus { _ =>
        accountingService.fetchAccountingDetails.map { accountingDetails =>
  ...
  val submit: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      accDForm.form
        .bindFromRequest()
        .fold(
          formWithErrors => Future.successful(BadRequest(view(formWithErrors, timeService.futureWorkingDate(SystemDate.getSystemDate, 60)))), {
