...
class SignInOutController @Inject() (val authConnector: PlayAuthConnector,
  ...
  def postSignIn(resend: Option[Boolean] = None, handOffID: Option[String] = None, payload: Option[String] = None): Action[AnyContent] =
  ...
  private def whenRegistrationIsDraft(throttleResponse: ThrottleResponse)(
      f: => Future[Result])(implicit hc: HeaderCarrier, req: Request[_]): Future[Result] =
    compRegConnector.fetchRegistrationStatus(throttleResponse.registrationId) flatMap {
      ...
      case Some(_) =>
        handOffService.cacheRegistrationID(throttleResponse.registrationId) map { _ =>
          Redirect(controllers.dashboard.routes.DashboardController.show)
        }
      case _ => f
  ...
  private def redirectToHo1WithCachedRegistrationId(regid: String)(implicit hc: HeaderCarrier, request: Request[_]) =
    handOffService.cacheRegistrationID(regid) map { _ =>
  ...
  private def checkHO5Progress(throttle: ThrottleResponse)(f: => Future[Result])(implicit hc: HeaderCarrier) = {
    ...
    throttle.registrationProgress match {
      case Some(HO5) =>
        cacheRegistrationID(throttle.registrationId).map { _ =>
  ...
  private[controllers] def processDeferredHandoff(optHandOffID: Option[String], optPayload: Option[String], throttleResponse: ThrottleResponse)(
      f: => Future[Result])(implicit hc: HeaderCarrier): Future[Result] = {
  ...
  def signOut(continueUrl: Option[RedirectUrl] = None): Action[AnyContent] = Action.async { implicit request =>
  ...
  def renewSession: Action[AnyContent] = Action.async { implicit request =>
    ...
    ctAuthorised {
      ...
      updateLastActionTimestamp()
        .map(_ => Ok.sendFile(new File("conf/renewSession.jpg")).as("image/jpeg").withHeaders(headers: _*))
  ...
  def timeoutShow: Action[AnyContent] = Action.async { implicit request =>
