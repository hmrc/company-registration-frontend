...
class BasicCompanyDetailsController @Inject() (val authConnector: PlayAuthConnector,
  ...
  // HO1
  val basicCompanyDetails: Action[AnyContent] = Action.async { implicit request =>
  ...
  // H01b
  def returnToAboutYou(request: String): Action[AnyContent] = Action.async { implicit _request =>
