...
class RegistrationConfirmationController @Inject() (
  ...
  // HO5.1 & old HO6
  def registrationConfirmation(requestData: String): Action[AnyContent] = Action.async { implicit request =>
  ...
  private[controllers] def successResponseHandling(regId: String, externalID: Option[String], payload: RegistrationConfirmationPayload)(implicit
      hc: HeaderCarrier,
    ...
    for {
      ...
      redirect <- confirmationResponse match {
        ...
        case ConfirmationReferencesSuccessResponse(_) =>
          Future.successful(Redirect(controllers.reg.routes.ConfirmationController.show).withLang(lang))
  ...
  private def getPaymentHandoffResult(externalID: Option[String], currentLanguage: String)(implicit
      hc: HeaderCarrier,
      request: MessagesRequest[AnyContent]): Future[Result] =
