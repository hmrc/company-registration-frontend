...
class GroupController @Inject() (val authConnector: PlayAuthConnector,
  ...
  // 3.2 hand off
  val PSCGroupHandOff: Action[AnyContent] = Action.async { implicit request =>
  ...
  // 3.1 handback
  def groupHandBack(requestData: String): Action[AnyContent] = Action.async { implicit request =>
  ...
  def pSCGroupHandBack(request: String): Action[AnyContent] = Action.async { implicit _request =>
  ...
  private[controllers] def pSCGroupHandBackSuccessRedirect(regId: String, payload: JsValue)(implicit
      hc: HeaderCarrier,
