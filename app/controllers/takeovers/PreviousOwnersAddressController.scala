...
import services.{AddressLookupFrontendService, AddressPrepopulationService, SessionCacheService, TakeoverService}
...
class PreviousOwnersAddressController @Inject() (val authConnector: PlayAuthConnector,
                                                 ...
                                                 view: HomeAddressView)(implicit val appConfig: AppConfig)
      ...
      checkStatus { regId =>
        takeoverService.getTakeoverDetails(regId).flatMap {
          ...
          case Some(TakeoverDetails(_, _, _, Some(previousOwnersName), optPreviousOwnerHomeAddress)) =>
            addressPrepopulationService.retrieveAddresses(regId).map { addressSeq =>
  ...
  val submit: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      registered { regId =>
        takeoverService.getTakeoverDetails(regId).flatMap { optTakeoverDetails =>
          ...
          (optTakeoverDetails, optAddressSeq) match {
            case (Some(TakeoverDetails(_, _, _, Some(previousOwnersName), _)), Some(addressSeq)) =>
              HomeAddressForm
                .form(addressSeq.length)
                .bindFromRequest()
                .fold(
                  formWithErrors =>
                    Future.successful(
                      BadRequest(view(formWithErrors, previousOwnersName, addressSeq ++ Seq(Messages("page.reg.ppob.differentAddress"))))),
                  {
                    case OtherAddress =>
                    ...
                    case PreselectedAddress(index) =>
                      takeoverService.updatePreviousOwnersAddress(regId, addressSeq(index)).map { _ =>
                        Redirect(regRoutes.AccountingDatesController.show).removingFromSession(addressSeqKey)
                      }
                  }
                )
