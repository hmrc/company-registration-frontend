...
class TestEndpointController @Inject() (val authConnector: PlayAuthConnector,
                                        ...
                                        viewDashboard: DashboardView)(implicit val appConfig: AppConfig, implicit val ec: ExecutionContext)
  ...
  private lazy val coHoURL: String =
  ...
  val getAllS4LEntries: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorisedOptStr(Retrievals.internalId) { internalID =>
      for {
        regID <- sessionCacheService.get[String]("registrationID") map {
          case Some(res) => res
        ...
        val companyDetailsForm = CompanyDetailsForm
          .form()
          .fill(
            companyDetails.getOrElse(
              CompanyDetails(
                "testCompanyName",
                CHROAddress("testPremises", "testAddressLine1", None, "testLocality", "UK", None, Some("ZZ1 1ZZ"), None),
                PPOB("RO", None),
        ...
        val handBackForm = FirstHandBackForm
          .form()
          .fill(handBackData match {
  ...
  val postAllS4LEntries: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
  ...
  val clearAllS4LEntries: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorisedOptStr(Retrievals.internalId) { internalID =>
      dataCacheService.clearCache(internalID) map { _ =>
        Ok(s"S4L for user oid $internalID cleared")
      }
    }
  }
  val clearKeystore: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorisedOptStr(Retrievals.internalId) { internalID =>
      sessionCacheService.clear() map { _ =>
        Ok(s"Keystore for user $internalID cleared")
      }
    }
  }
  val setupTestNavModel: Action[AnyContent] = Action.async { implicit request =>
    val nav = HandOffNavModel(
      Sender(
        nav = Map(
          ...
          "3" -> NavLinks(
            "http://localhost:9970/register-your-company/corporation-tax-summary",
            "http://localhost:9970/register-your-company/loan-payments-dividends"),
          "5" -> NavLinks(
            "http://localhost:9970/register-your-company/registration-confirmation",
            "http://localhost:9970/register-your-company/corporation-tax-summary")
        )
      ),
      Receiver(
        nav = Map(
          ...
          "2" -> NavLinks(
            "http://localhost:9986/incorporation-frontend-stubs/business-activities",
            "http://localhost:9986/incorporation-frontend-stubs/company-name-back"),
          "4" -> NavLinks(
            "http://localhost:9986/incorporation-frontend-stubs/incorporation-summary",
            "http://localhost:9986/incorporation-frontend-stubs/business-activities")
        ),
        jump = Map(
  ...
  val testEndpointSummary: Action[AnyContent] = Action.async { implicit request =>
    Future.successful(Ok(viewTestEndpointSummary()))
  }
  val fetchPrePopAddresses: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      registered { regId =>
  ...
  val fetchPrePopCompanyContactDetails: Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      registered { regId =>
  ...
  def showFeatureSwitch: Action[AnyContent] = Action.async { implicit request =>
  ...
  def updateFeatureSwitch(): Action[AnyContent] = Action.async { implicit request =>
  ...
  def simulateDesPost(ackRef: String): Action[AnyContent] = Action.async { implicit request =>
    dynStubConnector.simulateDesPost(ackRef).map(_ => Ok)
  }

  def verifyEmail(verified: Boolean): Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
  ...
  def dashboardStubbed(payeStatus: String = "draft",
                       ...
                       ctutr: Option[String] = None): Action[AnyContent] = Action { implicit request =>
    ...
    val payeDash =
      ServiceDashboard(payeStatus, Some("lastUpdateDate"), Some("ackrefPaye"), payeLinks, Some(dashboardService.getCurrentPayeThresholds))
    val vatLinks =
  ...
  private[controllers] def links(cancelUrl: Boolean, restartUrl: Boolean) =
    ServiceLinks("regURL", "otrsURL", if (restartUrl) Some("restartUrl") else None, if (cancelUrl) Some("cancelUrl") else None)

  def handOff6(transactionId: Option[String]): Action[AnyContent] = Action.async { implicit request =>
    ctAuthorised {
      registered { regId =>
