...
class HandOffServiceImpl @Inject() (val sessionCacheService: SessionCacheService,
                                    val encryptor: JweCommon,
                                    val appConfig: AppConfig,
                                    val navModelRepo: NavModelRepo,
                                    val compRegConnector: CompanyRegistrationConnector,
                                    val scrsFeatureSwitches: SCRSFeatureSwitches)(implicit val ec: ExecutionContext)
    extends HandOffService {

  lazy val returnUrl: String                = appConfig.self
  lazy val externalUrl: String              = appConfig.selfFull
  lazy val navModelMongo: NavModelRepoMongo = navModelRepo.repository
  lazy val timeout: Int                     = appConfig.timeoutInSeconds.toInt
  lazy val timeoutDisplayLength: Int        = appConfig.timeoutDisplayLength.toInt
}
...
trait HandOffService extends HandOffNavigator {
  ...
  private[services] val groupsCheckerForPSCHandOff = (groups: Option[Groups]) =>
    groups.map {
      case g @ Groups(true, name, addr, utr) if name.isDefined && addr.isDefined && utr.isDefined => g
      case g @ Groups(true, name, addr, utr) =>
        throw new Exception(s"[groupsChecker] Groups block incomplete, user skipped journey ${name.isDefined}, ${addr.isDefined}, ${utr.isDefined}")
      case g @ Groups(false, _, _, _) => Groups(groupRelief = false, None, None, None)
    }
  private[services] val buildTheStaticJumpLinksForGroupsBasedOnGroupsData = (groups: Option[Groups]) =>
    groups.map(og =>
      if (!og.groupRelief) {
        JumpLinksForGroups(
          groupReliefUrl,
  ...
  private[services] val buildParentCompanyNameOutOfGroups = (groups: Option[Groups]) =>
    groups.flatMap { og =>
      if (og.groupRelief) {
        Some(ParentCompany(og.nameOfCompany.get.name, og.addressAndType.get.address, og.groupUTR.get.UTR))
      } else {
        None
      }
    }
  ...
  def buildPSCPayload(regId: String, externalId: String, groups: Option[Groups], currentLanguage: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[Option[(String, String)]] = {
    val navModel = fetchNavModel() map { implicit model =>
      (forwardTo("3-2"), hmrcLinks("3-2"), model.receiver.chData)
    }

    navModel.map { navmodel =>
      val (url, links, chData) = navmodel
      val groupsInfluenced3bor3b1BackHandOff =
        (groups: Option[Groups]) => links.copy(reverse = groups.fold(regularPaymentsBackUrl)(groupsExists => groupBackHandBackUrl))
  ...
  def buildBusinessActivitiesPayload(regId: String, externalId: String, currentLang: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[Option[(String, String)]] = {
    val navModel = fetchNavModel() map { implicit model =>
  ...
  def companyNamePayload(regId: String, email: String, name: String, extID: String, currentLang: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[Option[(String, String)]] = {
    val navModel = fetchNavModel(canCreate = true) map { implicit model =>
  ...
  def summaryHandOff(externalID: String, currentLanguage: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[Option[(String, String)]] = {
    val navModel = fetchNavModel() map { implicit model =>
  ...
  def buildPaymentConfirmationHandoff(externalID: Option[String], currentLanguage: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[Option[(String, String)]] = {
    def navModel =
      fetchNavModel() map { implicit model =>
