...
class CompanyContactDetailsServiceImpl @Inject() (val businessRegConnector: BusinessRegistrationConnector,
    ...
    extends CompanyContactDetailsService
...
trait CompanyContactDetailsService extends CommonService with AuditService with SCRSExceptions with Logging {
  ...
  def fetchContactDetails(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[CompanyContactDetailsApi] = {
    ...
    val getVerifiedEmailAndContactDetails: Future[(String, CompanyContactDetailsResponse)] = for {
      ...
      verEmail <- companyRegistrationConnector.retrieveEmail(regId) map { e =>
        e.fold {
  ...
  def updateContactDetails(
      contactDetails: CompanyContactDetailsApi)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[CompanyContactDetailsResponse] =
    for {
  ...
  def checkIfAmendedDetails(email: String, cred: String, externalID: String, userContactDetails: CompanyContactDetails)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext,
