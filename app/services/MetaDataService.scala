...
trait MetaDataService extends CommonService with AuditService with SCRSExceptions with Logging {
  ...
  def updateCompletionCapacity(
      completionCapacity: AboutYouChoiceForm)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[BusinessRegistration] =
    for {
      regID    <- fetchRegistrationID
      updateCC <- businessRegConnector.retrieveAndUpdateCompletionCapacity(regID, getItemToSave(completionCapacity))
      _ = auditConnector.sendExplicitAudit(
        "SCRSRelationship",
        RelationshipIdentityVerificationAudit(Some(regID), Some(completionCapacity.completionCapacity)))
    } yield updateCC

  private def getItemToSave(completionCapacity: AboutYouChoiceForm): String =
  ...
  def getApplicantData(regId: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[AboutYouChoice] =
    businessRegConnector.retrieveMetadata(regId) map {
      case BusinessRegistrationSuccessResponse(resp) =>
        AboutYouChoice(resp.completionCapacity.getOrElse(throw new RuntimeException(s"Completion capacity missing at Summary for regId: $regId")))
      case unknown =>
