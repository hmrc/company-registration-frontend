...
class AddressLookupFrontendService @Inject()(addressLookupFrontendConnector: AddressLookupConnector,
  ...
  private[services] def messageKey(specificJourneyKey: String, mAPI: MessagesApi = messagesApi)(key: String): String = {
    ...
    if (mAPI.isDefinedAt(probableKey)) probableKey else potentialKey
  }
  ...
  private def continueUrl(call: Call): String = s"$companyRegistrationFrontendURL${call.url}"
  ...
  private[services] def initForGroups(regId: String, signOutUrl: String, call: Call)(implicit hc: HeaderCarrier): Future[JsObject] = {
    ...
    groupService.retrieveGroups(regId).map {
      optGroups =>
        ...
        val lookupJsObject = Json.obj("lookupPage" -> JsObject((initialJourneyConfig.lookupPageConfig \ "lookupPage").as[JsObject].fields.collect {
          partialFunc
        }))
        val confirmJsObject = Json.obj("confirmPage" -> JsObject((initialJourneyConfig.confirmPageConfig \ "confirmPage").as[JsObject].fields.collect {
          partialFunc
        }))
