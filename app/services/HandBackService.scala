...
trait HandBackService extends HandOffNavigator with Logging {
  ...
  private[services] def decryptHandBackRequest[T](request: String)(
      f: T => Future[Try[T]])(implicit hc: HeaderCarrier, formats: Format[T]): Future[Try[T]] =
  ...
  def processCompanyDetailsHandBack(request: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Try[CompanyNameHandOffIncoming]] = {
    ...
    decryptHandBackRequest[CompanyNameHandOffIncoming](request) { payload =>
      for {
  ...
  private[services] def storeCompanyDetails(payload: CompanyNameHandOffIncoming, regId: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[CompanyDetails] =
    updateCompanyDetails(regId, payload)

  private[services] def updateCompanyDetails(registrationID: String, handoff: CompanyNameHandOffIncoming)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[CompanyDetails] =
  ...
  def processGroupsHandBack(request: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Try[GroupHandBackModel]] = {
    ...
    decryptHandBackRequest[GroupHandBackModel](request) { payload =>
      payload.has_corporate_shareholders match {
        ...
        case Some(_) =>
          ...
          } yield cachedNavModel.fold[Try[GroupHandBackModel]](Failure(
            new Exception("[processGroupsHandBack] - Something went terribly wrong nav model did exist but upon updating it, it no longer exists")))(
            _ => Success(payload))
      }
    }
  }
  ...
  def processSummaryPage1HandBack(request: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Try[SummaryPage1HandOffIncoming]] = {
    ...
    decryptHandBackRequest[SummaryPage1HandOffIncoming](request) { payload =>
      for {
        model       <- fetchNavModel()
        _           <- processNavModel(model, payload)
        storeResult <- storeSimpleHandOff(payload)
      } yield storeResult match {
        case false =>
  ...
  private def updateNavModelWithLinksAndCHDataFromCOHO(handOff: String, links: NavLinks, ch: JsObject, oldModel: HandOffNavModel): HandOffNavModel =
    oldModel.copy(
      receiver = oldModel.receiver.copy(
  ...
  def decryptConfirmationHandback(request: String)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Try[RegistrationConfirmationPayload]] = {
    ...
    decryptHandBackRequest[RegistrationConfirmationPayload](request) { payload =>
  ...
  def storeConfirmationHandOff(payload: RegistrationConfirmationPayload, regID: String)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[ConfirmationReferencesResponse] =
