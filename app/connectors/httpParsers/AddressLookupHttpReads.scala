...
trait AddressLookupHttpParsers extends BaseHttpReads { _ : BaseConnector =>
  ...
  val addressHttpReads: HttpReads[NewAddress] = (_: String, url: String, response: HttpResponse) => response.status match {
    case status if is2xx(status) =>
      Try(response.json.as[NewAddress](NewAddress.addressLookupReads)) match {
        case Success(address) => address
        case Failure(e) =>
          logger.error("[addressHttpReads] NewAddress returned from ALF could not be parsed to Address model")
          throw e
      }
    case status =>
