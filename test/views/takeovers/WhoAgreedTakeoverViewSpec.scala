...
class WhoAgreedTakeoverViewSpec extends UnitSpec with GuiceOneAppPerSuite with I18nSupport {
  ...
  lazy val testBusinessName: String = "testName"

  "OtherBusinessNameView" should {
    lazy val form = WhoAgreedTakeoverForm.form
    lazy val view = views.html.takeovers.WhoAgreedTakeover(form, testBusinessName)
    lazy val doc = Jsoup.parse(view.body)

    lazy val title = s"Who agreed the takeover on behalf of $testBusinessName?"
    lazy val heading = s"Who agreed the takeover on behalf of $testBusinessName?"
