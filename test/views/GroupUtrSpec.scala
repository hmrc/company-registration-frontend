    ...
    "display the Group UTR page with no UTR pre-popped if no UTR in CR (first time through)" in new Setup {
      ...
      showWithAuthorisedUser(controller.show) {
        result =>
          ...
          document.title must include("Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?")
          document.getElementsByTag("h1").first().text() mustBe "Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?"
          document.getElementById("utr").attr("value") mustBe ""
          document.getElementsByTag("legend").text() mustBe "Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?"
      }
    }
    ...
    "display the Group UTR page with the UTR pre-popped if a UTR has already been saved in CR (second time through)" in new Setup {
      ...
      showWithAuthorisedUser(controller.show) {
        result =>
          ...
          document.title must include("Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?")
          document.getElementsByTag("h1").first.text() mustBe "Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?"
          document.getElementById("utr").attr("value") mustBe "1234567890"
          document.getElementsByTag("legend").text() mustBe "Do you know testGroupCompanyname1’s Unique Taxpayer Reference (UTR)?"
      }
    }
  }
